...
public class EngineBootstrap {
    ...
    public static void run(TwelfthApp app, AppConfig config) throws Exception {
        ...
        // ------------------------------------------------------------------
        RenderPipeline pipeline = app.onSetupRenderer(
                world, config, window, renderer2D, renderer3D, textRenderer);
        ...
        pipeline.addStep(RenderLayer.UI_2D_OVERLAY, ctx -> {
            ...
            if (paused[0]) {
                ...
                pauseMenu.drawButtons(
                        ctx.window(), ctx.renderer2D(), textRenderer,
                        InputManager.getMouseX(), InputManager.getMouseY());
            } else {
                // Crosshair
                ctx.renderer2D().setColor(1f, 1f, 1f, 1f);
                ctx.renderer2D().drawLine(centerX - crosshairSize, centerY, centerX + crosshairSize, centerY);
                ctx.renderer2D().drawLine(centerX, centerY - crosshairSize, centerX, centerY + crosshairSize);

                // TPS counter (bottom-right)
                String tpsText  = "TPS: " + CURRENT_TPS;
                float  tpsWidth = textRenderer.getTextWidth(tpsText, hudFontSize);
                textRenderer.drawText2D(tpsText, sw - tpsWidth - 16f, sh - 20f, hudFontSize, 1f, 1f, 1f, 1f);

                // Tick counter (bottom-left)
                textRenderer.drawText2D("Ticks: " + TICK_COUNTER, 16f, sh - 20f, hudFontSize, 1f, 1f, 1f, 1f);
            }
            ...
            if (showDebugOverlay[0] && !paused[0]) {
                ...
                textRenderer.drawText2D("12th Engine Debug (F3)", 16f, infoY, scale, 0.9f, 1f, 0.9f, 1f);
                infoY += 18f;
                ...
                if (cam != null) {
                    ...
                    textRenderer.drawText2D(
                            ...
                            16f, infoY, scale, 1f, 1f, 1f, 1f);
                    infoY += 16f;
                    textRenderer.drawText2D(
                            String.format(Locale.US, "Rot: Pitch %.2f  Yaw %.2f  Roll %.2f",
                ...
                textRenderer.drawText2D(
                        "FPS: " + CURRENT_FPS + " | TPS: " + CURRENT_TPS + " | Entities: " + world.getEntities().size(),
                        16f, infoY, scale, 1f, 1f, 1f, 1f);
                infoY += 16f;

                textRenderer.drawText2D(
                        String.format(Locale.US, "Frame: %.2f ms | Tick: %.2f ms | LagSim: %s",
        ...
        while (!window.shouldClose() && EngineObject.getInstance().isRunning()) {
            ...
            if (paused[0] && InputManager.isMousePressed(GLFW.GLFW_MOUSE_BUTTON_LEFT)) {
                PauseMenuScreen.Action action = pauseMenu.handleClick(
                        window, InputManager.getMouseX(), InputManager.getMouseY());
                if      (action == PauseMenuScreen.Action.RESUME) { paused[0] = false; window.lockMouse(); }
                else if (action == PauseMenuScreen.Action.EXIT)   { window.requestClose(); }
            }
            ...
            if (discordElapsed >= DISCORD_INTERVAL) {
                DiscordPresence.update(
                        "FPS: " + CURRENT_FPS + " | TPS: " + CURRENT_TPS,
