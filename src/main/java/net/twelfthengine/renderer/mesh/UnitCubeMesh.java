...
public class UnitCubeMesh {
    ...
    private static void face(float[] out, int base,
        ...
        int i = base;
        // tri 1
        putV(out, i, x1, y1, z1, nx, ny, nz, 0, 0); i += FLOATS_PER_VERTEX;
        putV(out, i, x2, y2, z2, nx, ny, nz, 1, 0); i += FLOATS_PER_VERTEX;
        putV(out, i, x3, y3, z3, nx, ny, nz, 1, 1); i += FLOATS_PER_VERTEX;
        // tri 2
        putV(out, i, x1, y1, z1, nx, ny, nz, 0, 0); i += FLOATS_PER_VERTEX;
        putV(out, i, x3, y3, z3, nx, ny, nz, 1, 1); i += FLOATS_PER_VERTEX;
        putV(out, i, x4, y4, z4, nx, ny, nz, 0, 1); i += FLOATS_PER_VERTEX;
    }

    private static void putV(float[] out, int i, float x, float y, float z, float nx, float ny, float nz, float u, float v) {
        out[i] = x;
        out[i + 1] = y;
        out[i + 2] = z;
        out[i + 3] = nx;
    ...
    private static float[] buildInterleavedCube() {
        ...
        float h = 0.5f;
        // +Z
        face(out, b, h, -h, h, 0, 0, 1, h, h, h, -h, h, h, -h, -h, h); b += 6 * FLOATS_PER_VERTEX;
        // -Z
        face(out, b, -h, -h, -h, 0, 0, -1, h, -h, -h, h, h, -h, -h, h, -h); b += 6 * FLOATS_PER_VERTEX;
        // +Y top
        face(out, b, -h, h, -h, 0, 1, 0, h, h, -h, h, h, h, -h, h, h); b += 6 * FLOATS_PER_VERTEX;
        // -Y bottom
        face(out, b, -h, -h, h, 0, -1, 0, h, -h, h, h, -h, -h, -h, -h, -h); b += 6 * FLOATS_PER_VERTEX;
        // +X
        face(out, b, h, -h, -h, 1, 0, 0, h, h, -h, h, h, h, h, -h, h); b += 6 * FLOATS_PER_VERTEX;
        // -X
        face(out, b, -h, -h, h, -1, 0, 0, -h, h, h, -h, h, -h, -h, -h, -h); b += 6 * FLOATS_PER_VERTEX;
        return out;
    }
    ...
    public void drawLit(ShaderProgram shader, Matrix4f model, Matrix4f view, Matrix4f projection,
                        Matrix4f lightSpace) {
        GL15.glBindBuffer(GL15.GL_ARRAY_BUFFER, vboId);
        int stride = FLOATS_PER_VERTEX * Float.BYTES;
        GL20.glEnableVertexAttribArray(0);
        ...
        shader.setUniformMatrix4fv("uLightSpaceMatrix", false, ShaderProgram.matrixToBuffer(lightSpace));
