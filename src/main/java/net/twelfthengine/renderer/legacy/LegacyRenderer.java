 ...
 *
 * Owns everything that uses:
 *   - glBegin / glEnd
 *   - GL_LIGHTING / GL_LIGHT0
 *   - glPushMatrix / glPopMatrix / glLoadMatrixf
 *   - glColor*, glNormal*, glVertex*, glTexCoord*
 *   - drawFilledBox, drawWireCube, drawLine, drawPoint, drawAxes
 *
 * Nothing in here should be touched when migrating to a modern pipeline.
 * To disable legacy rendering, simply stop calling this subsystem.
 */
public class LegacyRenderer {
    ...
    /**
     * Call once after the GL context is current to initialise the legacy
     * lighting model that was previously set up inside Renderer3D's constructor.
     */
    ...
    /**
     * Pushes projection + view matrices into the OpenGL fixed-function stack.
     * Call this before any legacy draw calls for a given frame.
     */
    ...
    /**
     * Renders every entity in the world using the fixed-function pipeline.
     * Equivalent to the old Renderer3D#renderLegacyScene.
     */
    public void renderScene(World world, ModelRenderer modelRenderer, TextureRenderer textureRenderer) {
    ...
    public void drawAxes(float length) {
        setLineWidth(2f);
        setColor(1, 0, 0, 1); drawLine(Vec3.ZERO, new Vec3(length, 0, 0));
        setColor(0, 1, 0, 1); drawLine(Vec3.ZERO, new Vec3(0, length, 0));
        setColor(0, 0, 1, 1); drawLine(Vec3.ZERO, new Vec3(0, 0, length));
        setColor(1, 1, 1, 1);
    }
    ...
    /**
     * Thin helper that handles the fixed-function matrix stack for a ModelEntity.
     * The actual VBO draw call still lives in VboModel — this class just wraps
     * the glPushMatrix/glPopMatrix dance that belongs to the legacy pipeline.
     */
