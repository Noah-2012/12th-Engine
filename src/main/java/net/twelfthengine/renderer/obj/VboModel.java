...
public class VboModel {
        ...
        for (Map.Entry<String, List<ObjModel.Face>> entry : groupedFaces.entrySet()) {
            ...
            for (ObjModel.Face face : faces) {
                for (int i = 0; i < 3; i++) {
                    ...
                    // Normale
                    var n = face.hasNormals ? model.normals.get(face.normalIndices[i]) : new net.twelfthengine.math.Vec3(0,1,0);
                    buffer.put(n.x()).put(n.y()).put(n.z());

                    // UV
                    var uv = face.hasUVs ? model.uvs.get(face.uvIndices[i]) : new net.twelfthengine.math.Vec2f(0,0);
     ...
     */
    public void renderLit(ObjModel sourceModel, ShaderProgram shader, Matrix4f model, Matrix4f view,
                          Matrix4f projection, Matrix4f lightSpace) {
        GL15.glBindBuffer(GL15.GL_ARRAY_BUFFER, vboId);
        int stride = 8 * 4;
        GL20.glEnableVertexAttribArray(0);
        GL20.glVertexAttribPointer(0, 3, GL11.GL_FLOAT, false, stride, 0);
