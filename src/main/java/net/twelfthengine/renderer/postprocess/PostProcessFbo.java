...
public class PostProcessFbo {
   ...
   *
   * <ul>
   *   <li>Color: {@code GL_RGBA8} / {@code GL_UNSIGNED_BYTE} — sufficient for LDR effects.
   *   <li>Depth: {@code GL_DEPTH_COMPONENT24} — faster than 32-bit on most hardware and accurate
   *       enough for all standard post-process depth reads (fog, DoF, etc.).
   * </ul>
   *
   * <p>For HDR pipelines, use {@link #PostProcessFbo(int, int, int, int)} and pass {@code
   * GL_RGBA16F} / {@code GL_FLOAT} as the color format arguments.
   */
  ...
  /**
   * Full-control constructor — lets you opt into HDR ({@code GL_RGBA16F}) or higher-precision depth
   * ({@code GL_DEPTH_COMPONENT32}) without changing the default path.
   *
   * @param colorInternalFormat e.g. {@code GL30.GL_RGBA8} or {@code GL30.GL_RGBA16F}
   * @param depthInternalFormat e.g. {@code GL14.GL_DEPTH_COMPONENT24} or {@code
  ...
  public PostProcessFbo(int width, int height, int colorInternalFormat, int depthInternalFormat) {
    ...
    boolean isHdr =
        (colorInternalFormat == GL30.GL_RGBA16F || colorInternalFormat == GL30.GL_RGBA32F);
